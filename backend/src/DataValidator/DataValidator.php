 ...
 * https://www.rafaelwendel.com/en/2013/01/datavalidator-php-class-to-validate-data/
 */
...
class Data_Validator
{
    ...
    protected function set_messages_default()
    {
        $this->_messages = array(
            ...
            'exact_length'   => 'O valor do campo %s deve ser igual a %s '
        );
    }
    ...
    public function exact_length($length, $inclusive = false)
    {
        $verify = ($inclusive === true ? strlen($this->_data['value']) != $length : strlen($this->_data['value']) == $length);
        if (!$verify) {
            $this->set_error(sprintf($this->_messages['exact_length'], $this->_data['name'], $length));
        }
        return $this;
    }
    ...
    {
        $verify = preg_match('/^[0-9]{8}$/', $this->_data['value']);
        if (!$verify) {
